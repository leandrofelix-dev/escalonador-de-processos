...
function srtf(modoExecucao, ordemfinalizacao, finalizacaoProcesso, idProcesso, tempoTotalProcessamento, tempo, tempoDeRetorno, mediaTempoDeRetorno, inicioProcesso, tempoEspera, mediaTempoEspera, tempoRestante) {
    ...
    let processosSrtfRajada = new Array;
    ...
    do {
        ...
        for (let i = 1; i < processos.length; i++) {
            ...
            if (tempo >= chegadaProcesso) {
                ...
                if (!processosSrtf.includes(idProcesso[1])) {
                    processosSrtf.push(idProcesso[1]);
                }
            }
        }

        // 2- Obter o tempo de rajada de cada processo da processosSrtf
        for (let i = 0; i < processosSrtf.length; i++){
            processo = processos[processosSrtf[i]].split(',');

            processosSrtfRajada.push(processos[processosSrtf[i]][2]);
        }
        console.log(tempo, "====", processosSrtfRajada);
